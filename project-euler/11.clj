...
(defn numify [l]
  (map #(if (string? %1) (Integer/parseInt %1) (numify %1)) l))
...
(defn adjacent-row [x y]
  (loop [i 0 acc []]
    (if (> i 3) acc
	(recur (inc i) (concat acc [(at (+ x i) y)])))))

(defn adjacent-col [x y]
  (loop [i 0 acc []]
    (if (> i 3) acc
	(recur (inc i) (concat acc [(at x (+ y i))])))))
...
(defn answer []
  (loop [y 0 acc []]
    (if (> y gridlen) (apply max acc)
	(recur (inc y) (concat acc (explore-row y))))))
